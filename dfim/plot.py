...
def plot_delta_tracks(track_list, title_list, x_tick_interval=1, 
                      track_height=3, axvline_loc=None,
                      plot_file=None):
    """
    track_list = list of tracks of size (seq_len, 4)
    title_list = list of labels corresponding to track_list
    """
    plt.clf()
    width = track_list[0].shape[1]
    fig_width = 20 + width/10
    fig = plt.figure(figsize=(fig_width, track_height*len(track_list)))
    yMax = np.max([arr.max() for arr in track_list]) + .01
    yMin = np.min([arr.min() for arr in track_list]) - .01
    for (i,array) in enumerate(track_list):
        ax = fig.add_subplot(len(track_list),1,i+1)
        if (array.shape[0]==4):
        ...
        ax.set_xticks(xticks_locations[::x_tick_interval])
        ax_2=ax.twiny()
    fig.tight_layout()
